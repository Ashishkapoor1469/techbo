...
import { Edit3Icon, MapPinIcon, LinkIcon, CalendarDaysIcon } from "lucide-react";
...
export default function ProfilePage() {
  const { 
    name, username, avatarUrl, dataAiHint, bio, postsCount, followersCount, 
    followingCount, joinedDate, location, websiteUrl,
    userPosts, userFrameworks, userPackages
  } = mockUserProfile;
          ...
          <div className="flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm text-muted-foreground">
                {location && <span className="flex items-center"><MapPinIcon className="mr-1.5 h-4 w-4" /> {location}</span>}
                {websiteUrl && <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center hover:text-primary"><LinkIcon className="mr-1.5 h-4 w-4" /> {websiteUrl.replace(/^https?:\/\//, '')}</a>}
                <span className="flex items-center"><CalendarDaysIcon className="mr-1.5 h-4 w-4" /> {joinedDate}</span>
              </div>
            </div>
          </div>
        </CardHeader>
        ...
        <TabsContent value="frameworks" className="mt-6">
           {userFrameworks && userFrameworks.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userFrameworks.map(fw => <ItemCard key={fw.id} item={fw} type="framework" />)}
            </div>
          ) : <p className="text-muted-foreground text-center py-8">No frameworks added yet.</p>}
        </TabsContent>
        <TabsContent value="packages" className="mt-6">
          {userPackages && userPackages.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userPackages.map(pkg => <ItemCard key={pkg.id} item={pkg} type="package" />)}
            </div>
